    ...
    for study_folder in os.listdir(patient_folder_path):
        ...
        for series_folder in os.listdir(study_folder_path):
            ...
            if open_dicom.Modality == "RTPLAN":
                ...
                if not LDR_brachy_case_verification.verify_if_brachy_treatment_type(dicoms_path, "HDR"):
                    stop_looking_in_study = True
                    break
                if not LDR_brachy_case_verification.verify_treatment_site(dicoms_path, "prostate"):
                    stop_looking_in_study = True
                    break
                if not LDR_brachy_case_verification.verify_if_source_corresponds_to_treatment_type(dicoms_path, "HDR"):
                    stop_looking_in_study = True
                    break
            ...
            if open_dicom.Modality == "RTSTRUCT":
                data_tracker.add_instance_uid_to_anonymized_uids_met(dicoms_path, random_str,  with_anonymization=False) # set to true
                if not contour_verification.verify_if_all_required_contours_are_present(dicoms_path, ["vessie"]):
                    stop_looking_in_study = True
                    break
